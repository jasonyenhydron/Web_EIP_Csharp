...
/*
 * GPanelTagHelper — 對應 jeasyui Panel
 * <g-panel title="基本資訊" icon="info" collapsible="true" collapsed="false">content</g-panel>
 */
namespace Web_EIP_Csharp.Views.Components
{
    ...
    public class GPanelTagHelper : TagHelper
    {
        ...
        /// <summary>
        /// 設為 true 時移除 overflow-hidden，允許內部絕對定位子元素（如 suggestion dropdown）超出 panel 邊界。
        /// 用法：<g-panel allow-overflow="true">
        /// </summary>
        ...
        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            ...
            var colBtn   = Collapsible
                ? $@"<button type=""button"" onclick=""gPanelToggle('{panelId}')"" title=""收合/展開""
            ...
            // AllowOverflow=true: 移除 overflow-hidden 以支援 suggestion dropdown 等絕對定位子元素
            var overflowCls  = AllowOverflow ? "overflow-visible" : "overflow-hidden";

            output.TagName = "div";
            output.Attributes.SetAttribute("class", $"bg-white rounded-xl border border-slate-200 shadow-sm {overflowCls} {Class}");
