...
{ // JQComboGrid -> ASP.NET Core TagHelper // Uses LOV interaction pattern in current stack. [HtmlTargetElement("g-combogrid")] public class GComboGridTagHelper : GLovInputTagHelper { [HtmlAttributeName("lov-function")] public string LovFunction { get; set; } = ""; [HtmlAttributeName("show-lov-button")] public bool? ShowLovButton { get; set; } public override void Process(TagHelperContext context, TagHelperOutput output) { if (string.IsNullOrWhiteSpace(LovFn) && !string.IsNullOrWhiteSpace(LovFunction)) { LovFn = LovFunction; } if (ShowLovButton.HasValue) { ShowButton = ShowLovButton.Value; } base.Process(context, output); } }
}
