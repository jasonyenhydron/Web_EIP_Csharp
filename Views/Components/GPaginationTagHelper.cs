...
/*
 * GPaginationTagHelper ??Â∞çÊ? jeasyui Pagination
 * Á∂ÅÂ? Alpine.js ËÆäÊï∏
    ...
    public class GPaginationTagHelper : TagHelper
    {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            var opts = PageSizeOptions.Split(',', StringSplitOptions.RemoveEmptyEntries)
                .Select(s => s.Trim())
                .Select(s => $"""<option :selected="{AlpinePageSize}=={s}" value="{s}">{s} Á≠???/option>""");
            var countHtml = !string.IsNullOrEmpty(AlpineCount)
                ? $"""<span class="text-slate-400 text-xs">??<span x-text="{AlpineCount}" class="font-bold text-slate-700"></span> Á≠?/span>"""
                : "";

            output.TagName = "div";
            output.Attributes.SetAttribute("class",
                "flex flex-wrap items-center justify-between gap-3 px-4 py-2 bg-slate-50/80 border-t border-slate-200 text-sm select-none");
