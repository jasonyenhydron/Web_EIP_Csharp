...
{
    /// <summary>
    /// g-empty-stateï¼šç©ºè³‡æ?? ä??«é¢?ƒä»¶
    /// ?¨è??™ç‚º?¶æ?é¡¯ç¤ºå±…ä¸­?–ç¤º + æ¨™é? + èªªæ??‡å???    ///
    /// ä½¿ç”¨?¹å?ï¼?    ///   @if (ViewBag.Programs?.Count == 0) {
    ///       <g-empty-state title="?¡ç¬¦?ˆæ?ä»¶ç?è³‡æ?"
    ///                      subtitle="è«‹èª¿?´ç¯©?¸æ?ä»¶å??æ–°?¥è©¢"
    ///                      icon="document" />
    ///   }
    ///
    /// Alpine.js æ¢ä»¶é¡¯ç¤ºï¼?    ///   <g-empty-state x-show="items.length === 0"
    ///                  title="å°šç„¡è³‡æ?" subtitle="è«‹å??·è??¥è©¢" />
    ///
    /// icon ?¯é¸ï¼šdocument | search | inbox | error
    /// </summary>
    [HtmlTargetElement("g-empty-state")]
    public class GEmptyStateTagHelper : TagHelper
    {
        /// <summary>ä¸»æ?é¡?/summary>
        public string Title { get; set; } = "?¡ç¬¦?ˆæ?ä»¶ç?è³‡æ?";

        /// <summary>?¯èªª?æ?å­?/summary>
        public string Subtitle { get; set; } = "è«‹èª¿?´ç¯©?¸æ?ä»¶å??æ–°?¥è©¢";

        /// <summary>icon é¡å?ï¼šdocument | search | inbox | error</summary>
        public string Icon { get; set; } = "document";

        /// <summary>å¤–å±¤ Alpine x-show è¡¨é?å¼ï??¯é¸ï¼?/summary>
