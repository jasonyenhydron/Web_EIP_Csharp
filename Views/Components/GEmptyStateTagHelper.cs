...
{
    /// <summary>
    /// g-empty-state：空資料占位畫面元件
    /// 在資料為零時顯示居中圖示 + 標題 + 說明文字。
    ///
    /// 使用方式：
    ///   @if (ViewBag.Programs?.Count == 0) {
    ///       <g-empty-state title="無符合條件的資料"
    ///                      subtitle="請調整篩選條件後重新查詢"
    ///                      icon="document" />
    ///   }
    ///
    /// Alpine.js 條件顯示：
    ///   <g-empty-state x-show="items.length === 0"
    ///                  title="尚無資料" subtitle="請先執行查詢" />
    ///
    /// icon 可選：document | search | inbox | error
    /// </summary>
    [HtmlTargetElement("g-empty-state")]
    public class GEmptyStateTagHelper : TagHelper
    {
        /// <summary>主標題</summary>
        public string Title { get; set; } = "無符合條件的資料";

        /// <summary>副說明文字</summary>
        public string Subtitle { get; set; } = "請調整篩選條件後重新查詢";

        /// <summary>icon 類型：document | search | inbox | error</summary>
        public string Icon { get; set; } = "document";

        /// <summary>外層 Alpine x-show 表達式（可選）</summary>
