...
namespace Web_EIP_Csharp.Views.Components
{
    ...
    public class GButtonTagHelper : TagHelper
    {
        ...
        // ─── 型別 → CSS class 對照表（Dictionary 取代 switch，方便擴充）───
        ...
        // ─── Icon SVG 庫 ────────────────────────────────────────
        /// <summary>依 icon 名稱回傳 SVG HTML；未知名稱回傳空字串。</summary>
        public static string GetIconSvg(string icon, string cls = "w-4 h-4 shrink-0")
        {
            ...
            // ─── Icon 路徑資料表（純資料，與渲染邏輯完全分離）────────────────
            // 格式：{ "icon名稱", (路徑陣列, 是否為 fill 模式) }
            // 單路徑：paths 只填一個元素；多路徑：填多個；fill 模式：isFill = true
            return icon switch
            {
