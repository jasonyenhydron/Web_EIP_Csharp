...
namespace Web_EIP_Csharp.Views.Components
{
    ...
    {
        public string Title { get; set; } = "Á≥ªÁµ±?ØË™§";
...
<div id=""gErrorMessageRoot"" class=""hidden fixed inset-0 z-[120] bg-slate-900/50 backdrop-blur-sm items-center justify-center p-4"">
  <div class=""w-full max-w-3xl bg-white border border-slate-200 rounded-xl shadow-2xl overflow-hidden"">
    <div class=""px-4 py-3 border-b border-slate-200 bg-slate-50 flex items-center justify-between"">
      ...
      <button type=""button"" class=""p-1.5 rounded hover:bg-slate-200 text-slate-500"" onclick=""gHideErrorMessage()"" title=""?úÈ?"">
        <svg class=""w-4 h-4"" fill=""none"" stroke=""currentColor"" viewBox=""0 0 24 24""><path stroke-linecap=""round"" stroke-linejoin=""round"" stroke-width=""2"" d=""M6 18L18 6M6 6l12 12""></path></svg>
      </button>
    </div>
    <div class=""p-4 space-y-3"">
      <div class=""text-sm text-slate-700"">
        <div><span class=""font-semibold"">Ë®äÊÅØÔº?/span><span id=""gErrMsgText""></span></div>
        <div><span class=""font-semibold"">‰æÜÊ?Ôº?/span><span id=""gErrMsgSource""></span></div>
        <div><span class=""font-semibold"">Ë°åË?Ôº?/span><span id=""gErrMsgLine""></span></div>
      </div>
      <div>
        <label class=""block text-xs text-slate-500 mb-1"">Ë©≥Á¥∞?ßÂÆπ</label>
      ...
      <div class=""flex justify-end gap-2"">
        <button type=""button"" onclick=""gCopyErrorMessage()"" class=""px-3 py-1.5 text-xs rounded border border-slate-300 bg-white hover:bg-slate-50 text-slate-700"">Ë§áË£Ω</button>
        <button type=""button"" onclick=""gHideErrorMessage()"" class=""px-3 py-1.5 text-xs rounded bg-blue-600 text-white hover:bg-blue-700"">?úÈ?</button>
      </div>
    </div>
  </div>
</div>
...
(function() {{
  ...
  function normalizeError(input) {{
    ...
    if (!input || typeof input !== 'object') {{
      ...
      return {{ message: t || '?™Áü•?ØË™§', source: '', lineNumber: '', detail: t }};
    }}
    return {{
      message: input.message || input.title || 'Á≥ªÁµ±?ºÁ??ØË™§',
  ...
  window.gCopyErrorMessage = async function() {{
    ...
    try {{
      ...
      if (window.gToast) window.gToast('?ØË™§?ßÂÆπÂ∑≤Ë?Ë£?, 'success');
    }} catch {{
  ...
  if (cfg.captureUnhandledRejection) {{
    window.addEventListener('unhandledrejection', function(ev) {{
      ...
      window.gShowErrorMessage({{
        message: reason.message || '?ûÈ???Promise ?ØË™§',
        source: '',
        lineNumber: '',
