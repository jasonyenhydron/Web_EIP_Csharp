...
<div id='gErrorMessageRoot' class='hidden fixed inset-0 z-[120] bg-slate-900/50 backdrop-blur-sm items-center justify-center p-4'>
  <div class='w-full max-w-3xl bg-white border border-slate-200 rounded-xl shadow-2xl overflow-hidden'>
    <div class='px-4 py-3 border-b border-slate-200 bg-slate-50 flex items-center justify-between'>
      ...
      <button type='button' class='p-1.5 rounded hover:bg-slate-200 text-slate-500' onclick='gHideErrorMessage()' title='Close'>
    ...
    <div class='p-4 space-y-3'>
      ...
      <div class='flex justify-end gap-2'>
        <button type='button' onclick='gCopyErrorMessage()' class='px-3 py-1.5 text-xs rounded border border-slate-300 bg-white hover:bg-slate-50 text-slate-700'>Copy</button>
        <button type='button' onclick='gHideErrorMessage()' class='px-3 py-1.5 text-xs rounded bg-blue-600 text-white hover:bg-blue-700'>Close</button>
      </div>
    </div>
  </div>
</div>
