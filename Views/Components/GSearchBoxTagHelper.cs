...
/*
 * GSearchBoxTagHelper ??Â∞çÊ? jeasyui ComboBox
 ...
 *               target-id="hidEmpId" placeholder="Ëº∏ÂÖ•?°Â∑•..."/>
 */
namespace Web_EIP_Csharp.Views.Components
{
    ...
    public class GSearchBoxTagHelper : TagHelper
    {
        ...
        public string Placeholder  { get; set; } = "Ëº∏ÂÖ•?úÈçµÂ≠?..";
                    ...
                    inp.addEventListener('input', function(){{
                        ...
                        _t = setTimeout(async ()=>{{
                            try{{
                                ...
                                const d = j.data ?? j;
                                lst.innerHTML = d.length ? '' : '<li class=""px-4 py-3 text-sm text-slate-400"">?°Á¨¶?àË???/li>';
                                d.forEach(item=>{{
