...
namespace Web_EIP_Csharp.Views.Components
{
    ...
    public class GProgressBarTagHelper : TagHelper
    {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            var stripedCls  = Striped ? @"bg-[size:1rem_1rem] bg-[linear-gradient(45deg,rgba(255,255,255,.15)_25%,transparent_25%,transparent_50%,rgba(255,255,255,.15)_50%,rgba(255,255,255,.15)_75%,transparent_75%,transparent)]" : "";
            ...
            output.Content.SetHtmlContent($@"
                <div class=""w-full bg-slate-200 rounded-full overflow-hidden h-4 relative"">
    ...
    public class GTooltipTagHelper : TagHelper
    {
        ...
        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            ...
            output.Content.SetHtmlContent($@"
                {content}
                <span class=""absolute {tooltipCls} z-[200] px-2 py-1 text-xs text-white bg-slate-700 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none shadow-lg"">
