...
{
    /// <summary>
    /// Alert message component for success, warning, info, and error states.
    /// </summary>
    [HtmlTargetElement("g-alert")]
    public class GAlertTagHelper : TagHelper
    {
        /// <summary>Alert type: error | success | warning | info.</summary>
        public string Type { get; set; } = "error";

        /// <summary>Alert message text.</summary>
        public string Message { get; set; } = string.Empty;

        /// <summary>Optional prefix text. If empty, use default by type.</summary>
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            // Style map by alert type
            (string baseCss, string iconSvg, string defaultPrefix) = Type?.ToLower() switch
            {
                "success" => (
                    "uk-background-primary border border-green-200 text-green-800",
                    @"<svg class=""w-5 h-5 text-green-500 shrink-0"" fill=""none"" stroke=""currentColor"" viewBox=""0 0 24 24"">
                        <path stroke-linecap=""round"" stroke-linejoin=""round"" stroke-width=""2"" d=""M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z""/></svg>",
                    "成功:"
                ),
                "warning" => (
                    "uk-background-muted border border-amber-200 text-amber-800",
                    @"<svg class=""w-5 h-5 text-amber-500 shrink-0"" fill=""none"" stroke=""currentColor"" viewBox=""0 0 24 24"">
                        <path stroke-linecap=""round"" stroke-linejoin=""round"" stroke-width=""2"" d=""M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z""/></svg>",
                    "警告:"
                ),
                "info" => (
                    "uk-background-primary border border-blue-200 text-blue-800",
                    @"<svg class=""w-5 h-5 text-blue-500 shrink-0"" fill=""none"" stroke=""currentColor"" viewBox=""0 0 24 24"">
                        <path stroke-linecap=""round"" stroke-linejoin=""round"" stroke-width=""2"" d=""M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z""/></svg>",
                    "資訊:"
                ),
                _ => (
                    "uk-background-muted border border-red-200 text-red-700",
