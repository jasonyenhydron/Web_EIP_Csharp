...
{
    /// <summary>
    /// g-combobox ??Â∞çÊ? jEasyUI ComboBoxÔºà‰??âÈÅ∏?áÊ?Ôº?    /// <g-combobox name="dept" label="?®È?" options="HR:‰∫∫Ë?,IT:Ë≥áË?,OP:?üÈ?" value="IT"/>
    /// ?ñÂ??úÊ??∏È?Âª∫Á?ÔºåÊ??èÈ? api-url ?ïÊ?ËºâÂÖ•
    /// </summary>
    ...
    public class GComboBoxTagHelper : TagHelper
    {
        ...
        public string Placeholder { get; set; } = "Ë´ãÈÅ∏??..";
        public string Options     { get; set; } = "";  // "value:text,value:text,..."
        public string ApiUrl      { get; set; } = "";   // ?ïÊ?ËºâÂÖ• API
    ...
    /// <summary>
    /// g-datebox ??Â∞çÊ? jEasyUI DateBoxÔºàÊó•?üÈÅ∏?áÔ?
    /// <g-datebox name="startDate" label="?ãÂ??•Ê?" value="2024-01-01" required="true"/>
    ...
    /// <summary>
    /// g-numberbox ??Â∞çÊ? jEasyUI NumberBoxÔºàÊï∏Â≠óËº∏?•Ê?Ôº?    /// <g-numberbox name="qty" label="?∏È?" min="0" max="9999" step="1" precision="0"/>
    /// </summary>
    ...
    public class GNumberBoxTagHelper : TagHelper
    {
        ...
        public int    Precision   { get; set; } = 0;  // Â∞èÊï∏‰ΩçÊï∏
        public string Prefix      { get; set; } = "";  // ?çÁΩÆÁ¨¶Ë?Â¶?$
        public string Suffix      { get; set; } = "";  // ÂæåÁΩÆÁ¨¶Ë?Â¶?%
        public string AlpineModel { get; set; } = "";
        public bool   Required    { get; set; } = false;
        public bool   Disabled    { get; set; } = false;
        public int    ColSpan     { get; set; } = 1;
            ...
            if (hasFix)
            {
                output.Content.SetHtmlContent($@"
                    ...
                    <div class=""flex items-center border border-slate-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-400 bg-white"">
                        {(string.IsNullOrEmpty(Prefix) ? "" : $@"<span class=""px-2 text-sm text-slate-500 bg-slate-50 border-r border-slate-300"">{Prefix}</span>")}
                        <input type=""number"" id=""{inputId}"" name=""{Name}"" value=""{Value}"" {attrs}
                               class=""flex-1 px-2 py-2 text-sm border-0 focus:outline-none bg-transparent"">
                        {(string.IsNullOrEmpty(Suffix) ? "" : $@"<span class=""px-2 text-sm text-slate-500 bg-slate-50 border-l border-slate-300"">{Suffix}</span>")}
                    </div>
                ");
            }
            else
    ...
    /// <summary>
    /// g-number-spinner ??Â∞çÊ? jEasyUI NumberSpinnerÔºàÂ∏∂?†Ê??âÈ??ÑÊï∏Â≠óÊ?Ôº?    /// <g-number-spinner name="qty" label="?∏È?" value="1" min="0" max="99"/>
    /// </summary>
            ...
            output.Content.SetHtmlContent($@"
                ...
                <div class=""flex items-center border border-slate-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-400 bg-white"">
                    ...
                    <button type=""button"" {disAttr}
                            ...
                            class=""px-2.5 py-2 text-slate-600 hover:bg-slate-100 transition-colors border-l border-slate-300 font-bold text-lg leading-none disabled:opacity-40"">+</button>
                </div>
            ");
        }
    }
