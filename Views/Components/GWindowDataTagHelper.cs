...
{
    /// <summary>
    /// g-messager — 對應 jEasyUI Messager（訊息彈窗）
    /// 提供全域 JS 方法：
    ...
    ///
    /// 在 Layout/_Layout.cshtml 放置一次即可：
    ///   <g-messager/>
    /// </summary>
    ...
    public class GMessagerTagHelper : TagHelper
    {
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            output.Content.SetHtmlContent($@"
                <!-- g-messager: Alert/Confirm/Prompt/Toast 容器 -->
                <div id=""gMsgOverlay"" class=""fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[900] hidden items-center justify-center"">
                ...
                const gMsg = (function() {{
                    const icons = {{
                        ...
                        error  : {{ bg:'bg-red-100',   color:'text-red-600',   svg:'<path stroke-linecap=""round"" stroke-linejoin=""round"" stroke-width=""2"" d=""M6 18L18 6M6 6l12 12""/>' }},
                        warning: {{ bg:'bg-amber-100', color:'text-amber-600', svg:'<path stroke-linecap=""round"" stroke-linejoin=""round"" stroke-width=""2"" d=""M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z""/>' }},
                        info   : {{ bg:'bg-blue-100',  color:'text-blue-600',  svg:'<path stroke-linecap=""round"" stroke-linejoin=""round"" stroke-width=""2"" d=""M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z""/>' }}
                    }};
                    ...
                    return {{
                        alert(title, msg, type='info', onOk) {{
                            show(title, msg, type, [mkBtn('確定','bg-blue-600 hover:bg-blue-700 text-white', ()=>{{ hide(); onOk&&onOk(); }})]);
                        }},
                        confirm(title, msg, onOk, onCancel, type='warning') {{
                            show(title, msg, type, [
                                mkBtn('取消','bg-slate-100 hover:bg-slate-200 text-slate-700', ()=>{{ hide(); onCancel&&onCancel(); }}),
                                mkBtn('確定','bg-blue-600 hover:bg-blue-700 text-white',       ()=>{{ hide(); onOk&&onOk();     }})
                            ]);
                        }},
                        prompt(title, defaultVal='', onOk, type='info') {{
                            ...
                            show(title, '', type, [
                                mkBtn('取消','bg-slate-100 hover:bg-slate-200 text-slate-700', ()=>hide()),
                                mkBtn('確定','bg-blue-600 hover:bg-blue-700 text-white',       ()=>{{ hide(); onOk&&onOk(inpEl.value); }})
                            ], true);
                        }},
                        toast(msg, type='success', duration=3000) {{
                            const tc = icons[type] || icons.info;
                            const bgMap = {{ success:'bg-green-600', error:'bg-red-600', warning:'bg-amber-500', info:'bg-blue-600' }};
                            const t = document.createElement('div');
                            t.className = `pointer-events-auto flex items-center gap-2 px-4 py-3 ${{bgMap[type]||'bg-slate-700'}} text-white rounded-xl shadow-lg text-sm font-medium translate-y-4 opacity-0 transition-all duration-300`;
    ...
    /// <summary>
    /// g-datalist — 對應 jEasyUI DataList（純清單顯示，不分頁）
    /// <g-datalist id="myList" api-url="/api/items" template="name:名稱:font-bold,code:代號:text-slate-400"/>
    ...
    public class GDataListTagHelper : TagHelper
    {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            output.Content.SetHtmlContent($@"
                ...
                <div x-show=""!loading && rows.length===0"" class=""py-8 text-center text-sm text-slate-400"">無資料</div>
    ...
    /// <summary>
    /// g-property-grid — 對應 jEasyUI PropertyGrid（屬性格顯示）
    /// <g-property-grid title="員工資訊">
    ///   <g-property name="員工編號" value="E001"/>
    ///   <g-property name="姓名" value="Jason Yen"/>
    /// </g-property-grid>
    /// </summary>
    ...
    public class GPropertyGridTagHelper : TagHelper
    {
        ...
        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            ...
            sb.Append(@"<colgroup><col class=""w-1/3 bg-slate-50""><col class=""w-2/3""></colgroup>");
            ...
            foreach (var (name, value, type) in pg.Rows)
            {
                ...
                <tr class=""border-b border-slate-200 last:border-0"">
                    <td class=""px-3 py-2.5 text-xs font-semibold text-slate-500 bg-slate-50 border-r border-slate-200"">{name}</td>
            ...
            var headerHtml = !string.IsNullOrEmpty(Title)
                ? $@"<div class=""px-4 py-2.5 bg-slate-50 border-b border-slate-200 text-sm font-bold text-slate-700"">{Title}</div>"
                : "";
    ...
    /// <summary>
    /// g-treegrid — 對應 jEasyUI TreeGrid（樹狀表格）
    /// <g-treegrid id="tg" api-url="/api/org/tree" id-field="id" pid-field="pid"
    ///             columns="name:名稱:200,code:代號:100,active:啟用:80:center"/>
    /// </summary>
    ...
    public class GTreeGridTagHelper : TagHelper
    {
        ...
        public int    PageSize { get; set; } = 0;   // 0=不分頁
                    ...
                    <table class=""min-w-full border-collapse"">
                        ...
                        <tbody>
                            <template x-for=""(row, idx) in visibleRows"" :key=""row['{IdField}']"">
                                <tr class=""{striped} hover:bg-blue-50/50 transition-colors {rowCursor}"" {rowClick}>
                                    {tdSb}
                                </tr>
                            </template>
                        </tbody>
                    </table>
                    <div x-show=""!loading && visibleRows.length===0"" class=""text-center py-8 text-sm text-slate-400"">無資料</div>
                </div>
                <script>
                function {fn}() {{
                    return {{
