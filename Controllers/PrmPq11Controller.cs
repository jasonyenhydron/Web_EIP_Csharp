// 功能：採購入庫查詢控制器，對應 prm_part_receipt_head / prm_part_receipt_detail 資料表，提供主檔與明細查詢。
// 輸入：查詢條件（廠商、託運單號、驗收單號區間、公司/部門/採購類型、日期區間、料號批號條件、狀態區間）。
// 輸出：PRMPQ11 主檔資料清單、單筆主檔對應明細資料清單（JSON）。
// 依賴：DbHelper、HttpContext.Session、Views/MisPrograms/PRMPQ11.cshtml、PrmPq11ViewModel。
...
namespace Web_EIP_Csharp.Controllers
{
    public class PrmPq11Controller : Controller
    {
        ...
        public IActionResult PRMPQ11()
        {
            ...
            var vm = new PrmPq11ViewModel();
            return View("~/Views/MisPrograms/PRMPQ11.cshtml", vm);
        }
