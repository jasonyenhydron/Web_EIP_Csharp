...
namespace Web_EIP_Csharp.Controllers
{
    public class FilesController : Controller
    {
        ...
        public IActionResult Index(string? path = null, string? q = null)
        {
            ...
            {
                model.ErrorMessage = "找不到可用的文件根目錄。";
                return View(model);
            }
            ...
            if (model.RequiresWindowsAuth)
            {
                ...
                {
                    model.ErrorMessage = "請先輸入 Windows Key 帳號密碼。";
                    return View(model);
                }

                if (!TryConnectShare(root, winUser, winPwd, out var authError))
                {
                    model.ErrorMessage = $"Windows 驗證失敗：{authError}";
                    return View(model);
                }
            ...
            {
                model.ErrorMessage = "目錄不存在或無權限存取。";
                return View(model);
            }
        ...
        public IActionResult Authenticate(string username, string password, string? domain, string? path = null, string? q = null)
        {
            ...
            {
                TempData["FilesAuthError"] = $"Windows 驗證失敗：{authError}";
