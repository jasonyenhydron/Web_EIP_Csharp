// 功能：MIS 樹狀選單控制器，提供選單頁面載入、建議搜尋與程式明細查詢 API。
// 輸入：輸入 program_no、q、employee_id、display_code 與 Session 連線資訊。
// 輸出：輸出 MISTreeMenu 視圖回應與選單、建議、明細的 JSON API 回應。
// 依賴：IdmProgramVModel、LovInputConfig、MISTreeMenu.cshtml、DbHelper、ASP.NET Core MVC。
                ...
                foreach (var program in programs)
                {
                    ...
                    else
                    {
                        if (!categories.ContainsKey("其他"))
                            categories["其他"] = new List<IdmProgramVModel>();
