...
namespace Web_EIP_Csharp.Controllers
{
    public class CmmController : Controller
    {
        ...
        [HttpGet("Cmm/CMMGD16")]
        public IActionResult CMMGD16()
        {
            if (!TryGetSessionCredentials(out var username, out _, out _))
            ...
            ViewBag.OrganizationId = DefaultOrganizationId;
...
WHERE rn > :offset AND rn <= :endRow");
            ViewBag.EmployeeLovApi = $"/api/lov/query?sql={employeeLovSql}";
