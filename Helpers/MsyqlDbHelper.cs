...
{
    // Class name follows requested naming: msyqldbhelper
    public static class MsyqlDbHelper
    {
        ...
        public static DbConnection CreateConnection(string? connectionString = null)
        {
            ...
            var conn = factory.CreateConnection() ?? throw new InvalidOperationException("Cannot create MySQL DbConnection.");
        ...
        {
            var factory = _providerFactory ?? ResolveFactory();
            var p = factory.CreateParameter() ?? throw new InvalidOperationException("Cannot create MySQL DbParameter.");
        ...
        public static DataTable GetDataTable(CommandType cmdType, string cmdText, params DbParameter[]? parameters)
        {
            ...
            using var cmd = conn.CreateCommand();
            PrepareCommand(cmd, conn, cmdType, cmdText, parameters);
            using var adapter = factory.CreateDataAdapter() ?? throw new InvalidOperationException("Cannot create MySQL DbDataAdapter.");
        ...
        private static DbProviderFactory ResolveFactory()
        {
            ...
            try { return DbProviderFactories.GetFactory("MySql.Data.MySqlClient"); } catch { }
            try { return DbProviderFactories.GetFactory("MySqlConnector"); } catch { }
            throw new InvalidOperationException("MySQL provider not found. Install MySql.Data or MySqlConnector.");
        }

        private static string GetDefaultConnectionString()
        {
            if (string.IsNullOrWhiteSpace(_defaultConnectionString))
                throw new InvalidOperationException("MsyqlDbHelper default connection string not configured.");
